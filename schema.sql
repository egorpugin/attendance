...
CREATE TABLE classes (lesson_id INTEGER PRIMARY KEY, teacher_id INTEGER REFERENCES person (person_id) ON DELETE CASCADE ON UPDATE CASCADE, subject_id INTEGER REFERENCES subject (subject_id) ON DELETE CASCADE ON UPDATE CASCADE);

-- ----------------------------
-- Table structure for groups
-- ----------------------------
DROP TABLE IF EXISTS "main"."groups";
CREATE TABLE groups (group_id INTEGER PRIMARY KEY AUTOINCREMENT, group_name TEXT NOT NULL, type_id INTEGER DEFAULT (1) NOT NULL,
UNIQUE ("group_name")
);
...
CREATE TABLE person_group (person_group_id INTEGER PRIMARY KEY, person_id INTEGER REFERENCES person (person_id) ON DELETE CASCADE ON UPDATE CASCADE, group_id INTEGER REFERENCES "group" (group_id) ON DELETE CASCADE ON UPDATE CASCADE);
